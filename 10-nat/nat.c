...
int parse_config(const char *filename)
{
    ...
    while (fgets(line, 256, fp)) {
        ...
        if (strncmp(line, IN_IFACE_STRING, delim - line) == 0) {
            sscanf(delim + 2, "%s", name);
            nat.internal_iface = if_name_to_iface(name);
        } else if (strncmp(line, EX_IFACE_STRING, delim - line) == 0) {
            sscanf(delim + 2, "%s", name);
            nat.external_iface = if_name_to_iface(name);
        } else if (strncmp(line, DNAT_RULES_STRING, delim - line) == 0) {
            u32 ex_ip, in_ip;
            u16 ex_port, in_port;
            if (sscanf(delim + 2, IP_FMT":%hu -> "IP_FMT":%hu",
                    HOST_IP_SCAN_STR(ex_ip), &ex_port,
                    HOST_IP_SCAN_STR(in_ip), &in_port) < 10) {
                log(ERROR, "illegal DNAT rule format");
                return 1;
            }
            struct dnat_rule *rule = malloc(sizeof(struct dnat_rule));
            rule->external_ip = ex_ip;
            rule->internal_ip = in_ip;
            rule->external_port = ex_port;
