...
struct tcp_sock *alloc_tcp_sock()
{
    ...
    // initialize send buffer and receive ofo buffer
    ...
    tsk->wait_empty = alloc_wait_struct();
    tsk->wait_full = alloc_wait_struct();
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
    ...
    struct tcp_sock *ptr;
    list_for_each_entry (ptr, &tcp_established_sock_table[idx], hash_list) {
...
// return 0: end of stream, connection lost
// return -1: error
// return positive: received length
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len) {
    ...
    while (ring_buffer_empty(tsk->rcv_buf)) {
        ...
        if (tsk->state == TCP_CLOSE_WAIT) {
            // log(DEBUG, "SOCK_READ: connection end, wake up to end everything");
            return 0;
        }

        // log(DEBUG, "SOCK_READ: ring buffer empty, go into sleep; STATE: %d", tsk->state);
        sleep_on(tsk->wait_empty);

        if (tsk->state == TCP_CLOSE_WAIT) {
            if (!ring_buffer_empty(tsk->rcv_buf)) {
                // log(DEBUG, "SOCK_READ: ring buffer got data, wake up to read");
                pthread_mutex_lock(&tsk->rcv_buf_lock);
            } else {
                // log(DEBUG, "SOCK_READ: connection end, wake up to end everything");
                return 0;
    ...
    // log(DEBUG, "read ring buffer, length: %d", rcv_len);
...
void tcp_sock_reset(struct tcp_sock *tsk) {
    tcp_bind_unhash(tsk);
...
int tcp_send_data(struct tcp_sock *tsk, char *buf, int len) {
    ...
    memcpy(pkt + ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE, buf, len);

    while (tsk->snd_wnd < len) {
        tsk->snd_wnd = 0;
        // log(DEBUG, "SOCK_WRITE: snd_wnd too small, go to sleep");
        sleep_on(tsk->wait_send);
        // log(DEBUG, "SOCK_WRITE: snd_wnd increases, let's see if it's enough");
    }

    tcp_send_packet(tsk, pkt, pkt_len);
    // log(DEBUG, "send TCP data, len: %d", len);
